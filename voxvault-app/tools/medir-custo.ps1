...
.PARAMETER Janela
    "aberta" or "minimizada". The ceiling is different for each, and the
    minimised figure is the one that matters most: for most of a meeting the
    window is not being looked at, and an interface that keeps working while
    hidden is spending the budget for nothing.
...
function Set-EstadoDaJanela {
    <#
      Drive the host window into the state being measured, instead of asking
      the operator to remember to do it. A measurement labelled "minimised"
      that was taken with the window open is worse than no measurement.
    #>
    param([string] $Estado)
    ...
    $comando = if ($Estado -eq "minimizada") { 6 } else { 9 }
    [void][VoxVault.Janela]::ShowWindow($hospedeiro.MainWindowHandle, $comando)
    Start-Sleep -Milliseconds 800
    $minimizada = [VoxVault.Janela]::IsIconic($hospedeiro.MainWindowHandle)
    return ($Estado -eq "minimizada") -eq $minimizada
}
...
function Get-Arvore {
    ...
    while ($fila.Count -gt 0) {
        ...
        if ($porPai.ContainsKey($atual.ProcessId)) {
            foreach ($filho in $porPai[$atual.ProcessId]) { $fila.Enqueue($filho) }
        }
    }
    ...
    foreach ($w in $webviewsForaDaArvore) { $vistos[$w.ProcessId] = $w }

    return $vistos
}

$limiteMemoria = if ($Janela -eq "minimizada") { $LIMITE_MEM_MINIMIZADA_MB } else { $LIMITE_MEM_ABERTA_MB }
...
Write-Host "  janela                : $Janela (limite de memoria $limiteMemoria MB)"
if (-not $estadoOk) {
...
while ((Get-Date) -lt $fim) {
    ...
    foreach ($processo in $arvore.Keys) {
        ...
        $ehServico = $arvore[$processo].ExecutablePath -like "*voxvault-core*"
        if ($ehServico) {
...
if ($amostras.Count -ge $porJanela) {
    for ($i = 0; $i -le $amostras.Count - $porJanela; $i++) {
        $janela = $amostras[$i..($i + $porJanela - 1)]
        $media = ($janela | Measure-Object CpuPct -Average).Average
...
Write-Host ("  processador, media do conjunto  : {0:N2}%   (limite {1}%)" -f $mediaCpu, $LIMITE_CPU_MEDIA)
...
Write-Host ""
Write-Host ("  veredito processador (media)    : {0}" -f $(if ($mediaCpu -le $LIMITE_CPU_MEDIA) { "DENTRO" } else { "ACIMA" }))
