...
- [x] 2.0.4 Registrar a decisão de backend e o resultado das medições, **incluindo a faixa de versão de Python que o backend escolhido impõe**. Resultado: o backend é `ctypes` sobre a API do sistema, sem extensão compilada, e portanto **não impõe teto algum**; o `requires-python` passou a `>=3.12`, com o piso vindo do `numpy`
