...
- **Do cache primeiro, e o núcleo sempre refeito.** Cada `sync` tenta primeiro com `--offline` e só vai à rede se faltar algo no cache: sem isso, o uv revalida no PyPI até o *build backend* que já tem guardado, e uma atualização sem dependência nova falharia numa máquina offline. E a etapa `dependencias` passa `--reinstall-package voxvault-core`: o uv só percebe que um projeto local mudou pelo `pyproject.toml`, e o instalador preserva a data de cada arquivo, então um núcleo alterado sem mudança no `pyproject` ficaria o antigo no ambiente. Verificado atualizando um ambiente da 0.1.0 para uma 0.1.1 de ensaio com todos os proxies numa porta morta: 7 s, o núcleo novo instalado e o cache do uv com os mesmos bytes, mais os metadados da roda refeita.
