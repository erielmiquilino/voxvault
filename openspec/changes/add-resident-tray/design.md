...
- **Eventos.** O serviço ganha `GET /eventos?desde=<seq>`, que devolve `{"eventos": [{"seq", "tipo", "uid", "detalhe", "instante"}], "ultimo": <seq>}`. O `seq` é monotônico por execução do serviço. `_record_event` e o callback `on_event` do pipeline passam a receber o `uid` separado do texto.
